...
# mask = np.nonzero(D < 0.5)

masked_DOS = np.array(D.copy())
mask = masked_DOS < 0.05
...
E_lim = 1.6
DOS_ax.set(ylim = ylim, xlim = (-E_lim, E_lim))
tau_ax.set(ylim = ylim, xlim = (-E_lim, E_lim))

DOS_ax.set(xticks = [])
tau_ax.set(xlabel = "Energy $E/J$")

fig.text(0.04, 0.5, r"Flux defect density $\rho$", ha = "right", va = "center", rotation = 90)
