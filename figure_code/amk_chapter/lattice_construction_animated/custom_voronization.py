...
def generate_lattice(
    ...
    if debug_plot is not False:
        ...
        plot_lines(ax, vor.vertices[inside_ridges], linewidths = linewidth, color = 'k')
        plot_lines(ax, vor.vertices[crossing_ridges], linewidths = linewidth, color = 'k', linestyle = '--')
        plot_lines(ax, vor.vertices[outer_ridges], linewidths = linewidth, color = 'k')
    ...
    # if debug_plot is not False:
    #     ax = axes[2]
    #     plot_lines(ax, vor.vertices[inside_ridges])
    #     for i,s in [[0,-1],[1,+1]]:
    #         a = vor.vertices[crossing_ridges].copy()
    #         a[:, i, :] = a[:, i, :] + s*adjacency_crossing
    #         plot_lines(ax, a, colors = 'r')
